...
function s.eqfilter(c)
	return c:IsFaceup() and c:IsCode(112345680) --ID do "JOJO Runo"
end
...
function s.btlcon(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    local bc=c:GetBattleTarget()
    if not bc then return false end
    local ec=c:GetEquipGroup():GetFirst()
    return ec and ec:IsCode(id) -- ainda está equipado com esta carta
end

function s.btlop(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    local bc=c:GetBattleTarget()
    if not bc or not c:IsRelateToBattle() then return end
    -- Impede dano
    local e1=Effect.CreateEffect(c)
    ...
    -- Impede o monstro do oponente de atacar até a próxima End Phase do oponente
    local e2=Effect.CreateEffect(c)
